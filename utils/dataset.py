...
# [[file:~/Works/char2token2mention/utils/dataset.org::*Import][Import:1]]
...
# [[file:~/Works/char2token2mention/utils/dataset.org::*YieldData][YieldData:1]]
class YieldData:
...
# YieldData:1 ends here

# Char2Token2MentionDS


# [[file:~/Works/char2token2mention/utils/dataset.org::*Char2Token2MentionDS][Char2Token2MentionDS:1]]
class Char2Token2MentionDS(Dataset):
    ...
    def gen_loss_args(self, n_names):
        ...
        dt_size = n_names.sum()
        label_range = n_names.cumsum(-1).repeat([dt_size, 1]).t()
...
# Char2Token2MentionDS:1 ends here

# Char2Token2MentionCEDS


# [[file:~/Works/char2token2mention/utils/dataset.org::*Char2Token2MentionCEDS][Char2Token2MentionCEDS:1]]
...
# Char2Token2MentionCEDS:1 ends here

# Char2Token2MentionWWDS


# [[file:~/Works/char2token2mention/utils/dataset.org::*Char2Token2MentionWWDS][Char2Token2MentionWWDS:1]]
class Char2Token2MentionWWDS(Dataset):
    def __init__(
        ...
        self.yield_data = self.yield_data_helper()
        all_word_code = list(
            chain(*[i["word_code"] for i in chain(*data.values())])
        )
        word_code_unique, word_code_count = torch.tensor(all_word_code).unique(
            return_counts=True
        )
        a = 1e-4
        self.word_weight = torch.zeros(word_code_unique.max())
        self.word_weight[word_code_unique - 1] = a / (
            a + word_code_count / word_code_count.sum()
        )
    ...
    def gen_data_fn_not_pad(self):
        ...
        n_words = torch.cat(
            [torch.tensor([r] * n) for n, r in zip(n_words, count(0))]
        )
        n_word_spm_args = {
            "indices": torch.stack([n_words, word_code]),
            "values": word_weight,
...
# Char2Token2MentionWWDS:1 ends here

# Char2MentionDS


# [[file:~/Works/char2token2mention/utils/dataset.org::*Char2MentionDS][Char2MentionDS:1]]
class Char2MentionDS(Char2Token2MentionDS):
    ...
    def pad_and_sort(inputs):
        inputs = pad_sequence(
            [torch.tensor(i) for i in inputs], batch_first=True,
        )
        lens = (inputs > 0).sum(-1)
    ...
    def gen_data_fn(self):
        batch_data, n_names = self.gen_data_helper([], [], 0)
        name_code, name_len, unsort_idx, _ = self.pad_and_sort(
            batch_data["name_code"]
        )
    ...
    def post_process(self, forward_args, loss_args, n_names):
        ...
        forward_args_new = {
            ...
            "name_code": forward_args["name_code"]
            .squeeze(0)
            .to(self.device)
            .split(split_n),
        }
...
# Char2MentionDS:1 ends here

# Char2MentionCEDS


# [[file:~/Works/char2token2mention/utils/dataset.org::*Char2MentionCEDS][Char2MentionCEDS:1]]
...
# Char2MentionCEDS:1 ends here

# Token2MentionDS


# [[file:~/Works/char2token2mention/utils/dataset.org::*Token2MentionDS][Token2MentionDS:1]]
class Token2MentionDS(Char2MentionDS):
    ...
    def prepare_eval_data(self, dt):
        _, n_word = (
            dt["n_word"].coalesce().indices()[0].unique(return_counts=True)
        )
...
# Token2MentionDS:1 ends here

# Token2MentionCEDS


# [[file:~/Works/char2token2mention/utils/dataset.org::*Token2MentionCEDS][Token2MentionCEDS:1]]
